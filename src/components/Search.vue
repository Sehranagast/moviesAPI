...
const recents = ref<string[]>(JSON.parse(localStorage.getItem('recents') || '[]') as string[])

const keyword = ref<string>(localStorage.getItem('keyword') ?? 'One Piece')
const isRecent = ref<boolean>(localStorage.getItem('isRecent') === 'true')
...
const filterRecent = (e: MouseEvent) => {
  const target = e.target as HTMLElement
  const recent = target.dataset.recent
  recents.value = recents.value.filter((item) => item !== recent)
  if (recents.value.length === 0) {
    isRecent.value = false
  }
}

const changeKeyword = (e: MouseEvent) => {
  const target = e.target as HTMLElement
  keyword.value = target.innerHTML
  store.getAllMovies(keyword.value)
  localStorage.setItem('keyword', target.innerHTML)
}
</script>
...
<template>
  ...
  <div class="mt-3 md:mt-5" v-if="isRecent">
    <div class="flex items-center">
      <h3 class="text-gray-200 text-sm md:text-base md:tracking-wider mr-3">Recent</h3>
      <div class="flex items-center mx-1 my-2 text-red-500 cursor-pointer" @click="clearRecent">
    ...
    <div class="flex mt-3 flex-wrap h-12 overflow-auto">
      <div
        ...
        :key="recent"
      >
        <span class="tracking-wide cursor-pointer md:text-lg" @click="changeKeyword">{{
          recent
        }}</span>
